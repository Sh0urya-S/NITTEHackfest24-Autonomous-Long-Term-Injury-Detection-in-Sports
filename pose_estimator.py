    ...
    # Define the key points positions
        ...
        for pair in self.POSE_PAIRS:
            ...
            if self.POINTS[idFrom] and self.POINTS[idTo]:
                ...
                cv2.line(frame, self.POINTS[idFrom], self.POINTS[idTo], (0, 255, 0), 3)
                    ...
                    if self.POINTS[self.BODY_PARTS["LShoulder"]] is not None and self.POINTS[self.BODY_PARTS["LElbow"]] is not None and self.POINTS[self.BODY_PARTS["LWrist"]] is not None:
                        ...
                        if a != 0 and b != 0:
                            try:
                                ...
                                if percentage > 50:
                                    percentage_color = (0, 0, 255)
                                if not math.isnan(percentage):
                                    print(f"{limb}: {percentage:.2f}%")
                                    cv2.putText(frame, f"{limb}: {percentage:.2f}%", (self.POINTS[self.BODY_PARTS["LElbow"]][0], self.POINTS[self.BODY_PARTS["LElbow"]][1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, percentage_color, 1)
                    ...
                    if self.POINTS[self.BODY_PARTS["RShoulder"]] is not None and self.POINTS[self.BODY_PARTS["RElbow"]] is not None and self.POINTS[self.BODY_PARTS["RWrist"]] is not None:
                        ...
                        if a != 0 and b != 0:    
                            try:
                                ...
                                if not math.isnan(percentage):
                                    print(f"{limb}: {percentage:.2f}%")
                                    cv2.putText(frame, f"{limb}: {percentage:.2f}%", (self.POINTS[self.BODY_PARTS["RElbow"]][0], self.POINTS[self.BODY_PARTS["RElbow"]][1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, percentage_color, 1)
                            except ValueError as e:
                                print(f"Error calculating angle for {limb}: {e}")
            elif limb == "LLeg":
                    ...
                    if self.POINTS[self.BODY_PARTS["LHip"]] is not None and self.POINTS[self.BODY_PARTS["LKnee"]] is not None and self.POINTS[self.BODY_PARTS["LAnkle"]] is not None:
                        ...
                        if a != 0 and b != 0:
                            try:
                                ...
                                if not math.isnan(percentage):
                                    print(f"{limb}: {percentage:.2f}%")
                                    cv2.putText(frame, f"{limb}: {percentage:.2f}%", (self.POINTS[self.BODY_PARTS["LKnee"]][0], self.POINTS[self.BODY_PARTS["LKnee"]][1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, percentage_color, 1)
                            except ValueError as e:
                                print(f"Error calculating angle for {limb}: {e}")
            elif limb == "RLeg":
                    ...
                    if self.POINTS[self.BODY_PARTS["RHip"]] is not None and self.POINTS[self.BODY_PARTS["RKnee"]] is not None and self.POINTS[self.BODY_PARTS["RAnkle"]] is not None:
                        ...
                        if a != 0 and b != 0:
                            try:
                                ...
                                if not math.isnan(percentage):
                                    print(f"{limb}: {percentage:.2f}%")
                                    cv2.putText(frame, f"{limb}: {percentage:.2f}%", (self.POINTS[self.BODY_PARTS["RKnee"]][0], self.POINTS[self.BODY_PARTS["RKnee"]][1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, percentage_color, 1)
                            except ValueError as e:
                                print(f"Error calculating angle for {limb}: {e}")
